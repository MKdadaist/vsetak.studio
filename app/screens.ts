...
export type Slot = {
  ...
  original?: string;
  // Центр кадра в процентах от исходника (уже повёрнутого).
  focus?: [number, number];
  // Масштаб кадра: 1 — вся картинка по короткой стороне, больше — приближение.
  zoom?: number;
  // Поворот исходника по часовой; rotated — повёрнутая копия для превью в админке.
  rotate?: 90 | 180 | 270;
  rotated?: string;
  alt?: string;
  title?: string;
